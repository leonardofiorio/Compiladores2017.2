...
--inside parser folder 'lua bin/luafish.lua -a examples/1.lua'

-- AST node type.
local ASTNode = {}
function ASTNode:__tostring() --checkHere
  local ts = {}
...
local function build_grammar(self)
  ...
  grammar.term =
    ...
    V'function' +
    Cf(Cg(V'prefixexp') * Cg(V'postfix' * 1)^0, postfix_helper) +   -- modified
    V'tableconstructor'
  ...
  --modified
  grammar.endindex =
    (V'postfix' * Cz(#V'postfix') * 1)^0 * V'postfixindex'

  -- modified (note: was left recusive)
  grammar.functioncall =
    (Cf(Cg(V'prefixexp') * Cg(V'postfix' * Cz(#V'postfix') * 1)^0, postfix_helper) * V'postfixcall') / postfix_helper
